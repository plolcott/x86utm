...
// The compiler refused to let me use bool
#define TRUE  1
#define FALSE 0
...
typedef struct {
  ...
  u32 NumBytes;
  u32 ESP; 
  u32 TOS; // Value on the top of the stack. 
  char Disassembly_text[256]; 
} LineOfCode;
...
unsigned x86emu_run(x86emu_t *emu, unsigned flags, int Disassemble) ATTRIBUTE1;                 // ATTRIBUTE1
void x86emu_stop(x86emu_t *emu);

void x86emu_set_log(x86emu_t *emu, unsigned buffer_size, x86emu_flush_func_t flush);
unsigned x86emu_clear_log(x86emu_t *emu, int flush) ATTRIBUTE1;                // ATTRIBUTE1
void x86emu_log(x86emu_t *emu, const char *format, ...) ATTRIBUTE2;            // ATTRIBUTE2
