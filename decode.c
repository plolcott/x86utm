...
//static void log_regs(x86emu_t *emu);  // Removed because of compiler warning 2022-08-06
//static void log_code(x86emu_t *emu);  // Removed because of compiler warning 2022-08-06
...
****************************************************************************/
API_SYM unsigned x86emu_run(x86emu_t *emu, unsigned flags, int Disassemble)  // 2020-05-17
{
  u8 op1, u_m1;
  s32 ofs32;
  char **p;
...
//  log_regs(emu);  // Write Registers to log 
    ...
    (*x86emu_optab[op1])(emu, op1);  // This is where the code is executed

    *emu->x86.disasm_ptr = 0; // NULL terminator of disasm_buf 
// 2021-05-03 disasm_buf filled just now 
...
//  for (int N = 0; N < sizeof(emu->line_of_code.Disassembly_text); N++) // 2022-09-07  Otto J. Makela Portability issue
    int N;                                                               // 2022-09-07  Otto J. Makela Portability issue
    for (N = 0; N < sizeof(emu->line_of_code.Disassembly_text); N++)     // 2022-09-07  Otto J. Makela Portability issue
...
//  printf("[%08x]%s\n", emu->x86.R_EIP, emu->line_of_code.Disassembly_text); 
  	...
  	if (Disassemble)                                             // 2020-07-22 
    {    // Force sequential disassembly of the whole code section
      emu->line_of_code.ESP = 0;                                 // 2020-09-20
      emu->x86.R_EIP = emu->x86.saved_eip + emu->x86.instr_len;  // 2020-09-20 
    }
...
//    printf("---Too Many Instructions(%lld)  (%lld)---\n", emu->x86.R_TSC, emu->max_instr);
...
/** Eliminate compiler warning  2022-08-06 
...
unsigned decode_memio(x86emu_t *emu, u32 addr, u32 *val, unsigned type)
{
  ...
  return err; // Prevents logging of Memory access 
